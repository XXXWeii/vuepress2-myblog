<template><div><p>相信用过 vue 的小伙伴，肯定被面试官问过这样一个问题：<strong>在 vue 中动态的引入图片为什么要使用 require？</strong></p>
...
<h2 id="_1-什么是静态资源" tabindex="-1"><a class="header-anchor" href="#_1-什么是静态资源" aria-hidden="true">#</a> 1.什么是静态资源</h2>
<hr>
...
<h2 id="_2-为什么动态添加的-src-会被当做的静态的资源" tabindex="-1"><a class="header-anchor" href="#_2-为什么动态添加的-src-会被当做的静态的资源" aria-hidden="true">#</a> 2. 为什么动态添加的 src 会被当做的静态的资源？</h2>
<hr>
...
<div class="language-vue line-numbers-mode" data-ext="vue"><pre v-pre class="language-vue"><code>// vue文件中动态引入一张图片
...
<h2 id="_3-没有进行编译-是指的是什么没有被编译" tabindex="-1"><a class="header-anchor" href="#_3-没有进行编译-是指的是什么没有被编译" aria-hidden="true">#</a> 3. 没有进行编译，是指的是什么没有被编译？</h2>
<hr>
...
<div class="language-vue line-numbers-mode" data-ext="vue"><pre v-pre class="language-vue"><code>// vue文件中静态的引入一张图片
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>template</span><span class="token punctuation">></span></span>
  <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>div</span> <span class="token attr-name">class</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>home<span class="token punctuation">"</span></span><span class="token punctuation">></span></span>
...
<figure><img src="@source/category/Vue/images/202304161.jpg" alt="图片" tabindex="0" loading="lazy"><figcaption>图片</figcaption></figure>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>  编译过后的src地址：../assets/logo.png
  编译过后的图片资源地址：/img/logo.6c137b82.png
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>那要怎么解决上述的问题呢，答案就是：require</p>
<h2 id="_4-加上-require-为什么能正确的引入资源-是因为加上-require-就能编译了" tabindex="-1"><a class="header-anchor" href="#_4-加上-require-为什么能正确的引入资源-是因为加上-require-就能编译了" aria-hidden="true">#</a> 4. 加上 require 为什么能正确的引入资源，是因为加上 require 就能编译了？</h2>
<hr>
<p>针对这个问题，首先就要否定后半句，无论加不加 require，vue 文件中引入一张图片都会被编译。</p>
<p>接着我们再来好好了解一下，require。</p>
<h4 id="_4-1-require-是什么-是一个-node-方法-用于引入模块-json-或本地文件" tabindex="-1"><a class="header-anchor" href="#_4-1-require-是什么-是一个-node-方法-用于引入模块-json-或本地文件" aria-hidden="true">#</a> 4.1 require 是什么: 是一个 node 方法，用于引入模块，JSON 或本地文件</h4>
<h4 id="_4-2-调用-require-方法引入一张图片之后发生了什么" tabindex="-1"><a class="header-anchor" href="#_4-2-调用-require-方法引入一张图片之后发生了什么" aria-hidden="true">#</a> 4.2 调用 require 方法引入一张图片之后发生了什么：</h4>
...
<figure><img src="@source/category/Vue/images/202304162.jpg" alt="图片" tabindex="0" loading="lazy"><figcaption>图片</figcaption></figure>
...
<p>答案就是这么简单，来验证一波</p>
<div class="language-vue line-numbers-mode" data-ext="vue"><pre v-pre class="language-vue"><code>// vue文件中使用require动态的引入一张图片
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>template</span><span class="token punctuation">></span></span>
  <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>div</span> <span class="token attr-name">class</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>home<span class="token punctuation">"</span></span><span class="token punctuation">></span></span>
...
//这张图片是可以被正确打开的
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>img</span> <span class="token attr-name">src</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>/img/logo.6c137b82.png<span class="token punctuation">"</span></span> <span class="token attr-name">alt</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>logo<span class="token punctuation">"</span></span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="@source/category/Vue/images/202304163.jpg" alt="图片" tabindex="0" loading="lazy"><figcaption>图片</figcaption></figure>
...
<h2 id="_5-问题-3-中-静态的引入一张图片-没有使用-require-为什么返回的依然是编译过后的文件地址" tabindex="-1"><a class="header-anchor" href="#_5-问题-3-中-静态的引入一张图片-没有使用-require-为什么返回的依然是编译过后的文件地址" aria-hidden="true">#</a> 5. 问题 3 中，静态的引入一张图片，没有使用 require，为什么返回的依然是编译过后的文件地址？</h2>
<hr>
<p>答：在 webpack 编译的 vue 文件的时候，遇见 src 等属性会默认的使用 require 引入资源路径。引用 vue-cli 官方的一段原话</p>
<blockquote>
<p>当你在 JavaScript、CSS 或 <code v-pre>*.vue</code> 文件中使用相对路径 (必须以 <code v-pre>.</code> 开头) 引用一个静态资源时，该资源将会被包含进入 webpack 的依赖图中。在其编译过程中，所有诸如 <code v-pre>&lt;img src=&quot;...&quot;&gt;</code>、<code v-pre>background: url(...)</code> 和 CSS <code v-pre>@import</code> 的资源 URL <strong>都会被解析为一个模块依赖</strong>。</p>
<p>例如，<code v-pre>url(./image.png)</code> 会被翻译为 <code v-pre>require('./image.png')</code>，而：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>&lt;img src="./image.png">
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>将会被编译到：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>h('img', { attrs: { src: require('./image.png') }})
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></blockquote>
<h2 id="_6-按照问题-5-中所说-那么动态添加-src-的时候也会使用-require-引入-为什么-src-编译过后的地址-与图片资源编译过后的资源地址不一致" tabindex="-1"><a class="header-anchor" href="#_6-按照问题-5-中所说-那么动态添加-src-的时候也会使用-require-引入-为什么-src-编译过后的地址-与图片资源编译过后的资源地址不一致" aria-hidden="true">#</a> 6. 按照问题 5 中所说，那么动态添加 src 的时候也会使用 require 引入，为什么 src 编译过后的地址，与图片资源编译过后的资源地址不一致</h2>
<hr>
<p>答：因为动态引入一张图片的时候，src 后面的属性值，实际上是一个变量。webpack 会根据 v-bind 指令去解析 src 后面的属性值。并不会通过 reuqire 引入资源路径。这也是为什么需要手动的添加 require。</p>
<h2 id="_7-据说-public-下面的文件不会被编译-那我们使用静态路径去引入资源的时候-也会默认的使用-require-引入吗" tabindex="-1"><a class="header-anchor" href="#_7-据说-public-下面的文件不会被编译-那我们使用静态路径去引入资源的时候-也会默认的使用-require-引入吗" aria-hidden="true">#</a> 7.据说 public 下面的文件不会被编译，那我们使用静态路径去引入资源的时候，也会默认的使用 require 引入吗？</h2>
<hr>
<p>官方的原文是这样子的：</p>
<blockquote>
<p>任何放置在 <code v-pre>public</code> 文件夹的静态资源都会被简单的复制，而不经过 webpack。你需要通过绝对路径来引用它们。</p>
</blockquote>
<p>答：不会，使用 require 引入资源的前提的该资源是 webpack 解析的模块，而 public 下的文件压根就不会走编译，也就不会使用到 require。</p>
<h2 id="_8-为什么使用-public-下的资源一定要绝对路径" tabindex="-1"><a class="header-anchor" href="#_8-为什么使用-public-下的资源一定要绝对路径" aria-hidden="true">#</a> 8.为什么使用 public 下的资源一定要绝对路径</h2>
<hr>
...
<h2 id="_9-上文件中提到的-webpack-为什么引入资源的时候要有-base64-和打包到-dist-目录下两种的方式-全部打包到的-dist-目录下-他不香吗" tabindex="-1"><a class="header-anchor" href="#_9-上文件中提到的-webpack-为什么引入资源的时候要有-base64-和打包到-dist-目录下两种的方式-全部打包到的-dist-目录下-他不香吗" aria-hidden="true">#</a> 9.上文件中提到的 webpack，为什么引入资源的时候要有 base64 和打包到 dist 目录下两种的方式，全部打包到的 dist 目录下，他不香吗？</h2>
<hr>
<p>答：为了减少 http 请求。页面中通过路径引入的图片，实际上都会向服务器发送一个请求拿到这张图片。对于资源较小的文件，设置成 base64，既可以减少请求，也不会影响到页面的加载性能。</p>
<ul>
<li>参考链接：<a href="http://cli.vuejs.org/zh/" target="_blank" rel="noopener noreferrer">cli.vuejs.org/zh/<ExternalLinkIcon/></a></li>
<li>参考链接：<a href="http://wjhsh.net/vickylinj-p-15599154.html" target="_blank" rel="noopener noreferrer">wjhsh.net/vickylinj-p-15599154.html<ExternalLinkIcon/></a></li>
</ul>
</div></template>

<template><div><figure><img src="@source/category/JavaScript/images/scroll.png" alt="上拉加载下拉刷新" tabindex="0" loading="lazy"><figcaption>上拉加载下拉刷新</figcaption></figure>
...
<h2 id="二、实现原理" tabindex="-1"><a class="header-anchor" href="#二、实现原理" aria-hidden="true">#</a> 二、实现原理</h2>
<hr>
...
<p>首先可以看一张图</p>
<figure><img src="@source/category/JavaScript/images/isscroll.png" alt="上拉加载下拉刷新原理" tabindex="0" loading="lazy"><figcaption>上拉加载下拉刷新原理</figcaption></figure>
<p>上拉加载的本质是页面触底，或者快要触底时的动作</p>
<p>判断页面触底我们需要先了解一下下面几个属性</p>
<ul>
<li><code v-pre>scrollTop</code>：滚动视窗的高度距离<code v-pre>window</code>顶部的距离，它会随着往上滚动而不断增加，初始值是 0，它是一个变化的值</li>
<li><code v-pre>clientHeight</code>:它是一个定值，表示屏幕可视区域的高度；</li>
<li><code v-pre>scrollHeight</code>：页面不能滚动时是不存在的，<code v-pre>body</code>长度超过<code v-pre>window</code>时才会出现，所表示<code v-pre>body</code>所有元素的长度</li>
</ul>
<p>综上我们得出一个触底公式：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code>scrollTop <span class="token operator">+</span> clientHeight <span class="token operator">>=</span> scrollHeight<span class="token punctuation">;</span>
...
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code>_element<span class="token punctuation">.</span><span class="token function">addEventListener</span><span class="token punctuation">(</span>
...
<h2 id="三、案例" tabindex="-1"><a class="header-anchor" href="#三、案例" aria-hidden="true">#</a> 三、案例</h2>
<hr>
